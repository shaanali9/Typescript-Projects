...
const Calculate = async () => {
    ...
    const operandOne = input.numberOne;
    const operandTwo = input.numberTwo;
    const _Operator = input.operator;
    const validateInput = isNumber(operandOne) && isNumber(operandTwo) && isOperator(_Operator);
    let result = 0;
    if (validateInput) {
        switch (_Operator) {
            case "+":
